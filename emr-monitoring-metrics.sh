#!/bin/sh
tee -a /home/hadoop/emr-montoring-metrics.sh > /dev/null <<EOT
...
cat << EOF | curl --data-binary @- ${pushgateway}${instance_ip}/type/${namespace_prefix}
    memory_total ${mem[0]}
    memory_free ${mem[1]}
    memory_used ${mem[2]}
EOF
...
EOF
EOT
chmod 600 /home/hadoop/emr-montoring-metrics.sh
